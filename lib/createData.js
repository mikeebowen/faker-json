...
function createData(val, parentKeys) {
  if (typeof val === 'string') {
  ...
  } else if (['boolean', 'number'].includes(typeof val) || val == null ) {
    return val;
    ...
    if (count) {
      ...
      for (let i = 0; i < count; i++) {
        if (!val.hasOwnProperty('__useVal')) {
          const newObj = {};

          Object.keys(val).forEach(k => {
            if (!k.startsWith('__')) {
              newObj[k] = createData(callIf(val[k]), `${parentKeys}.${k}`);
            }
          });
          ret.push(newObj);
        } else {
          ret.push(createData(callIf(val.__useVal)));
        }
      }
    } else if (Array.isArray(val)) {
      ret = val.map((v, i) => createData(callIf(v), `${parentKeys}[${i}]`));
    } else {
      ret = {};
      Object.keys(val).forEach(k => {
        if (!k.startsWith('__')) {
          ret[k] = createData(callIf(val[k]), `${parentKeys}.${k}`);
        }
      });
    }
